...
contract ZirconPylon {
    ...
    function initPylon(address _to) external lock {
        ...
        //TODO: Old definition, necessary because pool may not be initialized but check for weird interactions
    ...
    function mintAsync100(address to, bool isAnchor) isInitialized external returns (uint liquidity) {
        ...
        //TODO: Bug, this mints pTokens before actually doing the swap

        if (isAnchor) {
            reserve = reserve1;
            balance = IERC20Uniswap(pair.token1).balanceOf(address(this));
            console.log("<<<_mintAsync100: Calling _mintPoolToken");
